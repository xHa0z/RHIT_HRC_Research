...
#     thrid_frame.place(x=150,y=150)
    ...
    instruction_box.insert(1.0, '1.) Please place the blocks in the cubby holes according to how they are positioned on '
                                    'the GUI.' + '\n' + '\n'
                                '2.) Press the Start NLP button to begin talking to the robot. Say the color of the block and When you are done pause for a '
...
#         consent_window(None)
#     def time_check():
#         time_check.close_loop = root.after(2000, Force_Quit)
#         time_check.timer_root = Tkinter.Toplevel()
#         timer_frame = ttk.Frame(time_check.timer_root, padding=(25, 25))
#         timer_frame.grid()
#         timer_label = ttk.Label(timer_frame, text='You have 5 seconds to click continue to stop the game from restarting.')
#         timer_label.grid()
...
#         loop = root.after(3000, time_check)
#           
#     loop = root.after(3000, time_check)
    ...
    Leap_Motion_Button['command'] = lambda: n(text_box, canvas, root)
...
#         text_box.update_idletasks()
    ...
    def nlp_check():
        ...
        if nlp_process.poll() != None:
#             print(nlp_process.poll())
...
#             command = 'taskkill /F /pid ' + str(pid)
            ...
            loop = root.after(1000, nlp_check)
...
#             root.after_cancel(loop)
            ...
            loop = root.after(1000, leap_check)
    
    loop = root.after(1000, leap_check)
#     subprocess.Popen(leap_check(leap_process, root), shell =True)
#     leap_process.wait()
#     text_box.configure(background='green')
#     text_box.update_idletasks()

#     bktask = BackgroundTask(Leap_Motion(text_box, canvas, root, Wait_Timer))
#     bktask.start()
