...
########
        ...
        if not frame.tools.is_empty:  # check if there is a tool
            ...
            if a == 1:
                ...
                ypos = frame.tools[0].tip_positio0n[1] + mag * frame.tools[0].direction[1]
...
def compute_prob(xo=0, yo=0, sd=200):
    yo = -yo
    sd_left = (-200 - xo) / sd
    sd_right = (200 - xo) / sd
    sd_bottom = (-200 - yo) / sd
...
def main():
    ...
    """prob = compute_prob(xo=avg_xpar_sum, yo=avg_ypar_sum - 200, sd=100)
